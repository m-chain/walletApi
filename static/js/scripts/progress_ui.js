...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
  ...
  if (!this.fileProgressWrapper.find('td:eq(0) .btn-default').length) {
...
FileProgress.prototype.setComplete = function(up, info) {
  var td = this.fileProgressWrapper.find('td:eq(0)'),
  ...
  td.find(".imgDiv").append(Wrapper);

  if (!isImg) {
    showImg.attr('src', '/static/img/default.png');
    ...
    $(img).on('load', function() {
      ...
      linkWrapper.attr('href', originUrl).attr('title', '查看原图');
...
FileProgress.prototype.setCancelled = function(manual) {
  ...
  this.fileProgressWrapper.find('td:eq(0) .btn-default').hide();
...
FileProgress.prototype.bindUploadCancel = function(up) {
  var self = this;
  if (up) {
    self.fileProgressWrapper.find('td:eq(0) .progressCancel').on('click',
      function() {
